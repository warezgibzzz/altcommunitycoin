...
void Shutdown()
{
    ...
    RenameThread("altcommunitycoin-shutoff");
    ...
    if (pwalletMain)
        bitdb.Flush(true);
    Generatealtcommunitycoins(false, NULL, 0);
#endif
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: altcommunitycoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: altcommunitycoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 26855 or testnet: 36855)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 25855 or testnet: 35855)") + "\n";
    ...
    strUsage += "\n" + _("SSL options: (see the altcommunitycoin Wiki for SSL setup instructions)") + "\n";
...
/** Sanity checks
 *  Ensure that altcommunitycoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. altcommunitycoin is shutting down."));
...
#endif
    // Make sure only a single altcommunitycoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. altcommunitycoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("altcommunitycoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "altcommunitycoin server starting\n");
    ...
    // requested to kill altcommunitycoin-qt during the last operation. If so, exit.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of altcommunitycoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart altcommunitycoin to complete") << "\n";
    ...
    if (pwalletMain)
        Generatealtcommunitycoins(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", -1));
#endif
