...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("altcommunitycoin-miner");
...
void static altcommunitycoinMiner(CWallet *pwallet)
{
    LogPrintf("altcommunitycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("altcommunitycoin-miner");
	...
	CBlock *pblock = pblocktemplate.get();
        ...
        LogPrintf("Running altcommunitycoinMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("altcommunitycoinMiner terminated\n");
        throw;
    }
}

void Generatealtcommunitycoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
	...
	minerThreads->create_thread(boost::bind(&altcommunitycoinMiner, pwallet));
}
#endif
