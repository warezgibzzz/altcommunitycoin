...
#if defined(NDEBUG)
# error "altcommunitycoin cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 4 * 60 * 60; // 4 hour(s)
...
const string strMessageMagic = "altcommunitycoin Signed Message:\n";
...
unsigned int DarkGravityWave(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Modified for altcommunitycoin: softfork block set to min diff to re-engage chain movement
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    //LogPrintf("altcommunitycoin retargetted using: DGW-v3 difficulty algo \n");
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal altcommunitycoin miner
...
#ifdef ENABLE_WALLET
// altcommunitycoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("altcommunitycoin-loadblk");
