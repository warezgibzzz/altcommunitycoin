...
#include "altcommunitycoinaddressvalidator.h"
...
QFont altcommunitycoinAddressFont()
{
...
{
    widget->setMaxLength(altcommunitycoinAddressValidator::MaxAddressLength);
    widget->setValidator(new altcommunitycoinAddressValidator(parent));
    widget->setFont(altcommunitycoinAddressFont());
}
...
bool parsealtcommunitycoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("altcommunitycoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!altcommunitycoinUnits::parse(altcommunitycoinUnits::ALTCOM, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parsealtcommunitycoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert altcommunitycoin:// to altcommunitycoin:
    //
    //    Cannot handle this later, because altcommunitycoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("altcommunitycoin://"))
    {
        uri.replace(0, 12, "altcommunitycoin:");
    }
    QUrl uriInstance(uri);
    return parsealtcommunitycoinURI(uriInstance, out);
}
...
void openConfigfile()
{
    ...
    /* Open altcommunitycoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "altcommunitycoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for altcommunitycoin.lnk
...
{
    return GetAutostartDir() / "altcommunitycoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a altcommunitycoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=altcommunitycoin\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("altcommunitycoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  altcommunitycoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("altcommunitycoin-Qt"));
