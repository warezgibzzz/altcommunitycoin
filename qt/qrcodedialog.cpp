...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("INFO")
    updateDisplayUnit();
}
...
{
    QString ret = QString("altcommunitycoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non INFO unit input in lnReqAmount, we generate the URI with INFO as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(altcommunitycoinUnits::format(altcommunitycoinUnits::INFO, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
